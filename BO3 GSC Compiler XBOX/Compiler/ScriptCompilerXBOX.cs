...
namespace GSCCompilerXBOX
{
    public partial class ScriptCompiler
    {
        ...
        public static Dictionary<string, short> StringPositionDict; //dictionary of all strings and their offsets
        ...
        private void SetAlignedWord(int offset = 0)
        {
        ...
        private void SetAlignedDword(int offset = 0)
        {
        ...
        {
            str = (str + "\0").ToLower();
            Int64 hash = (str[0] ^ 0xE5770569) * 0x1000193;
            for (int z = 1; z < str.Length; z++)
                hash = (char.ToLower(str[z]) ^ hash) * 0x1000193;
            return (uint)(hash & 0x00000000FFFFFFFF); //remove bytes past the uint32 limit and convert to uint
        }

        private void AddString(string str)
        {
            if (CompiledPub.Count > ushort.MaxValue)
            {
                OutputString += "ERROR: String Pointer is bigger than 65535! You have too many very long strings!" + System.Environment.NewLine;
                ErrorInScript = true;
            }
            StringPositionDict.Add(str, (short)CompiledPub.Count);
        ...
        private void AddRefToCall(uint FunctionNameHash, byte NumOfParams, byte Flags, uint FileNameHash, string namestring) //add to imports
        {
            ...
            {
                Import.Refs.Add((CompiledPub.Count - 1)); //-1 as imports reference the opcode start
                Import.NumOfRefs++;
                return;
            }
            ...
            Imports.Add(new Import
            {
                ...
                Flags = Flags,
                Refs = new List<int> { CompiledPub.Count - 1 }
            });
        ...
        private void AddRefToString(string str)
        {
            ...
            {
                if(StringTableDict[str].NumOfRefs + 1 > byte.MaxValue)
                {
                    OutputString += "ERROR: More than 255 of the same string has been used!" + System.Environment.NewLine;
                    ErrorInScript = true;
                }
        ...
        {
            CompiledPub.AddRange(BitConverter.GetBytes(i).Reverse().ToArray());
        }
        ...
        {
            CompiledPub.AddRange(BitConverter.GetBytes(f).Reverse().ToArray());
        }

        private void AddUshort(ushort u)
        {
            CompiledPub.AddRange(BitConverter.GetBytes(u).Reverse().ToArray());
        }

        private void AddShort(short s)
        {
            CompiledPub.AddRange(BitConverter.GetBytes(s).Reverse().ToArray());
        }
        ...
        private void ResetAllVars()
        {
            ...
            StringPositionDict = new Dictionary<string, short>();
        ...
        public bool Init(ParseTree Tree, bool OutputFunctions)
        {
            ...
            file.NamePtr = (ushort)CompiledPub.Count;
            ...
            file.Magic = new byte[] { 0x80, 0x47, 0x53, 0x43, 0x0D, 0x0A, 0x00, 0x03 };
            file.SourceCRC = CRC;
        ...
        private void WriteAnimTreesToFile()
        {
            foreach(AnimTree animtree in AnimTrees)
            {
                ...
                {
                    AddInt(animref.AnimTreeNamePtr);
                    AddInt(animref.AnimTreeRef);
                }
            }
        }
        ...
        private void EmitGetAnimation(ParseTreeNode node)
        {
            ...
            AddInt(GetStringPosByName(str)); //gets overwritten during linking
        }
        ...
        private void EmitGetString(string str, bool isIString)
        {
            ...
            AddShort(GetStringPosByName(str));
        }
        ...
        private void EmitCall(ParseTreeNode callNode, bool decTop) //dectop if it is a call not wrapped in another call
        {
            ...
            uint namehash = 0;
            string namestring = "";
            if (callNode.ChildNodes[baseCallNodeIndex].ChildNodes[1].Term.Name == "identifier") //check if it is a method call
            {
            ...
            switch (callNode.Term.Name)
            {
                case "scriptFunctionCall":
                    if(!IsGSCForFunction && BuiltinHashList.Contains(namehash)) //360 requires OP_CallBuiltin to be used, unlike PC which can use OP_ScriptFunctionCall for builtins
                        EmitOpcode(OP_CallBuiltin);
                    else
                        EmitOpcode(OP_ScriptFunctionCall);
                ...
                case "scriptMethodCall":
                    ...
                    if (!IsGSCForFunction && BuiltinHashList.Contains(namehash)) //360 requires OP_CallBuiltinMethod to be used, unlike PC which can use OP_ScriptMethodCall for builtins
                        EmitOpcode(OP_CallBuiltinMethod);
                    else
                ...
                case "CallBuiltin": //this is only here because my old compiler used ::: to declare builtins and this adds support for it
                    EmitOpcode(OP_CallBuiltin);
                    flag |= (byte)FunctionFlags.FunctionCall;
                    break;

                case "CallBuiltinMethod":
                    EmitOwner(callNode.ChildNodes[0]);
                    EmitOpcode(OP_CallBuiltinMethod);
                    flag |= (byte)FunctionFlags.MethodCall;
                    break;
        ...
        private short GetStringPosByName(string str)
        {
